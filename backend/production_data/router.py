...
class ProductionRecord(BaseModel):
    """
    Represents a single production record.
    """
    id: str = Field(..., example="prod_rec_001", description="Unique ID for the production record")
    product_name: str = Field(..., example="Widget A", description="Name of the product produced")
    machine_id: str = Field(..., example="Machine-001", description="ID of the machine used")
    operator_id: str = Field(..., example="Op-005", description="ID of the operator")
    quantity_produced: int = Field(..., example=150, gt=0, description="Number of units produced")
    start_time: datetime = Field(..., example="2025-07-09T08:00:00Z", description="Production start timestamp")
    end_time: datetime = Field(..., example="2025-07-09T08:30:00Z", description="Production end timestamp")
    quality_status: str = Field(..., example="Passed", description="Quality check status (e.g., Passed, Failed)")
    notes: Optional[str] = Field(None, example="Minor adjustment made during run", description="Any relevant notes")

class ProductionRecordCreate(BaseModel):
    """
    Model for creating a new production record. ID is generated by the API.
    """
    product_name: str = Field(..., example="Gadget B", description="Name of the product produced")
    machine_id: str = Field(..., example="Machine-002", description="ID of the machine used")
    operator_id: str = Field(..., example="Op-007", description="ID of the operator")
    quantity_produced: int = Field(..., example=200, gt=0, description="Number of units produced")
    start_time: datetime = Field(..., example="2025-07-09T09:00:00Z", description="Production start timestamp")
    end_time: datetime = Field(..., example="2025-07-09T09:45:00Z", description="Production end timestamp")
    quality_status: str = Field(..., example="Passed", description="Quality check status (e.g., Passed, Failed)")
    notes: Optional[str] = Field(None, example="Smooth operation", description="Any relevant notes")

class ProductionRecordUpdate(BaseModel):
    """
    Model for updating an existing production record. All fields are optional.
    """
    product_name: Optional[str] = Field(None, example="Super Widget", description="Updated product name")
    machine_id: Optional[str] = Field(None, example="Machine-001", description="Updated machine ID")
    operator_id: Optional[str] = Field(None, example="Op-006", description="Updated operator ID")
    quantity_produced: Optional[int] = Field(None, example=160, gt=0, description="Updated quantity produced")
    start_time: Optional[datetime] = Field(None, example="2025-07-09T08:00:00Z", description="Updated start timestamp")
    end_time: Optional[datetime] = Field(None, example="2025-07-09T08:40:00Z", description="Updated end timestamp")
    quality_status: Optional[str] = Field(None, example="Failed", description="Updated quality status")
    notes: Optional[str] = Field(None, example="Required recalibration", description="Updated notes")
...
async def create_production_record(
    ...
    current_user: UserResponse = Depends(role_required(["supervisor", "admin","operator"])) # SUPERVISOR OR ADMIN
):
    """
    Creates a new production record. A unique ID will be generated.
    Requires 'supervisor' or 'admin' role.
    """
    new_id = f"prod_rec_{len(mock_production_db) + 1:03d}" # Simple ID generation
    new_record = ProductionRecord(id=new_id, **record.model_dump())
    mock_production_db[new_id] = new_record
    return new_record

@router.get(
    "/",
    response_model=List[ProductionRecord],
    summary="Get All Production Records"
)
async def get_all_production_records(
    product_name: Optional[str] = Query(None, description="Filter by product name (case-insensitive partial match)"),
    machine_id: Optional[str] = Query(None, description="Filter by machine ID (case-insensitive partial match)"),
    operator_id: Optional[str] = Query(None, description="Filter by operator ID (case-insensitive partial match)"),
    quality_status: Optional[str] = Query(None, description="Filter by quality status (e.g., 'Passed', 'Failed')"),
    ...
    sort_by: Optional[str] = Query("start_time", description="Field to sort by (e.g., 'start_time', 'quantity_produced', 'product_name')"),
    sort_order: Optional[str] = Query("desc", description="Sort order: 'asc' for ascending, 'desc' for descending"),
    current_user: UserResponse = Depends(role_required(["operator","supervisor", "admin"])) # SUPERVISOR OR ADMIN
):
    """
    Retrieves all production records, with optional filters and sorting.
    Requires 'supervisor' or 'admin' role.
    """
    ...
    # The DEBUG prints are good for verification, you can keep them if you want
    for record in filtered_records:
        if record.start_time.tzinfo is None or record.end_time.tzinfo is None:
            print(f"DEBUG: Found naive datetime in record {record.id} before sorting!")
            print(f"DEBUG: Start Time: {record.start_time}, End Time: {record.end_time}")

    # --- Sorting Logic ---
    valid_sort_fields = [
        "product_name", "machine_id", "operator_id",
        "quantity_produced", "start_time", "end_time", "quality_status"
    ]
    if sort_by not in valid_sort_fields:
        sort_by = "start_time" # Default to 'start_time' if invalid field is provided

    # THIS IS THE CRUCIAL CHANGE: Apply make_datetime_aware to the sort key
    if sort_by:
        # Check if the sort_by field is a datetime field, and if so, make it aware for comparison
        if sort_by in ["start_time", "end_time"]:
            ...
            # For other fields (strings, numbers), direct getattr is fine
            filtered_records.sort(
...
async def get_production_record_by_id(
    ...
    """
    Retrieves a single production record by its unique ID.
    Requires 'supervisor' or 'admin' role.
    """
    ...
    if not record:
        raise HTTPException(
            ...
            detail=f"Production record '{record_id}' not found."
        )
    return record
...
async def update_production_record(
    ...
    """
    Updates an existing production record by its ID.
...
async def delete_production_record(
    ...
    """
    Deletes a production record by its unique ID.
    Requires 'supervisor' or 'admin' role.
    """
    if record_id not in mock_production_db:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Production record '{record_id}' not found."
        )
    del mock_production_db[record_id]
    # FastAPI requires a return, even for 204. Can return None or an empty dict.
